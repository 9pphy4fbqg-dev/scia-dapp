...
#### 问题描述
- 用户连接钱包后，系统错误地显示"未注册"状态
- API请求返回HTML而不是JSON，导致前端解析错误
- 前端和后端端口配置不一致，导致CORS问题

#### 解决方案
1. 配置Vite代理，将/api请求转发到http://localhost:3001
2. 将所有API请求从绝对路径改为相对路径
3. 确保前端运行在3000端口，后端运行在3001端口

#### 实施细节
- 修改文件：vite.config.ts、src/pages/Profile.tsx、src/pages/Layout.tsx
- 修复内容：
  - 在vite.config.ts中添加代理配置
  - 将Profile.tsx中的7个API请求改为相对路径
  - 将Layout.tsx中的1个API请求改为相对路径
- 版本标签：v1.1.6